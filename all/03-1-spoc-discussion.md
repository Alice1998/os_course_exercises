...
这几个过程不是完全分开的，动态加载的库通常在加载的过程中进行链接。

2.动态链接如何使用？尝试在Linux平台上使用LD_DEBUG查看一个C语言Hello world的启动流程。  (optional)

- 通过LD_DEBUG =help查看如何使用, LD_DEBUG=all ./hello 完成查看

### 5.3 连续内存分配
1.什么是内碎片、外碎片？

- 内碎片：分配给任务的内存大小比任务所需的空间多出来的内存。
- 外碎片：分配给任务的内存˙之间，无法利用的内存。

2.最先匹配会越用越慢吗？请说明理由（可来源于猜想或具体的实验）？

- 最先匹配会先找低地址空间的内存，到后期低地址空间都是大量的小的，不连续的内存空间，每次都要扫描低地址空间后，才能到达空地址空间，接触到可用的内存。

3.最差匹配的外碎片会比最优适配算法少吗？请说明理由（可来源于猜想或具体的实验）？

- 最差匹配过程中，每次都找最大的内存块进行分割，因此剩下的内存块也很大，往往还能够装下一个程序，所以应该不会比最优适配算法差。
